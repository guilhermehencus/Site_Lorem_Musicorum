...
$result_c = "select*from usuario where id_usuario_login=1 order by nome LIMIT $inicial, $quantidade_pg";
...
?>
	...
	<head>
		...
		<script type="text/text/javascript" src="../../js/jquery-3.4.1.min.js"></script>
...
<?php
	$result_pg = "SELECT count(id) as 'total'FROM usuario where id_usuario_login=1";
	...
	echo "<span class='page-link'> <a href='#' onclick='pagina_aluno(1, $quantidade_pg)' >  Primeiro</a> </span>";
	echo '</li>';
	...
	for ($pagina_dep = $pagina + 1; $pagina_dep <= $pagina + $max_links; $pagina_dep++) {
		if ($pagina_dep <= $row_result_pg) {
			echo "<li class='page-item'> <a class='page-link' href='#' onclick='pagina_aluno($pagina_dep, $quantidade_pg)' > $pagina_dep </a> </li>";
		}
	}
	echo '<li class="page-item">';
	echo "<span class='page-link'><a href='#' onclick='pagina_aluno($row_result_pg, $quantidade_pg)'>Última</a></span>"; /* <span class='page-link'><a href='#' onclick='pagina_aluno($row_result_pg, $quantidade_pg)'>Última</a></span> */
	echo '</li>';
	echo '</ul>';
	echo '</nav>';
	/* cpmparando */
